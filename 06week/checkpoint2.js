...
const nums = strNums.map(n => parseInt(n))
...
let weatherStates =  []
//define function isANewState to test if a given weather state does not yet exist in weatherStates
let isANewState = (string) => {
    let returnState = true
    weatherStates.map(function(element) {
      if (element == string) {
        returnState = false
      }  
    })
...
//iterating through weather, if a weather state does not yet exist in weatherStates, push it into that array
weather.map(function(element) {
    if (isANewState(element.weather_state_name)) {
